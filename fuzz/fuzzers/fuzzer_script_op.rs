...
fuzz_target!(|input: &[u8]| {
    ...
    let (_all_ops, packet) = input.split_at(64);
    let (ops, rr_ops) = input.split_at(32);
    let mut cloned_packet = packet.to_vec();
    let cloned_packet = cloned_packet.as_mut();
    ...
    for op in ops {
        ...
        let _ = match *op {
            ...
            9 => match parsed.into_iter_question() {
            ...
            10 => match parsed.into_iter_answer() {
                Some(mut i) => match i.next() {
                    Some(mut i) => iter_ops(&mut i, rr_ops),
                    None => 1,
                },
            ...
            11 => match parsed.into_iter_nameservers() {
                Some(mut i) => match i.next() {
                    Some(mut i) => iter_ops(&mut i, rr_ops),
                    None => 1,
                },
            ...
            12 => match parsed.into_iter_additional() {
                Some(mut i) => match i.next() {
                    Some(mut i) => iter_ops(&mut i, rr_ops),
                    None => 1,
                },
            ...
            13 => match parsed.into_iter_additional_including_opt() {
                Some(mut i) => match i.next() {
                    Some(mut i) => iter_ops(&mut i, rr_ops),
                    None => 1,
                },
            ...
            14 => match parsed.into_iter_edns() {
                Some(mut i) => match i.next() {
                    Some(mut i) => iter_ops(&mut i, rr_ops),
                    None => 1,
                },
            ...
            26 => {
                ...
                if maybe_parsed.is_ok() {
                    ...
                    let _cloned_dns_sector = DNSSector::new(packet).unwrap();
                    1
                } else {
                    0
                }
